...
const StatsBar = ({ pageName, partnerId }: StatsBarProps) => {
  ...
  const gridColumns = {
    ...
    5: 'md:grid-cols-5',
    6: 'md:grid-cols-3'
  }[pageStats.length] || 'md:grid-cols-4'

  return (
    <article className={`min-h-[200px] grid grid-cols-1 ${gridColumns} gap-8 px-6 md:px-4 py-8 max-w-7xl mx-auto`}>
      {isPending ? (
        ...
        pageStats.map((stat: StatItem, index: number) => (
          <div
            key={stat.key}
            className={`flex flex-col items-center text-center pb-6 md:pb-0 ${index !== pageStats.length - 1 ? 'border-b md:border-b-0' : ''} border-black`}
          >
